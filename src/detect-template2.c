...
static int PrefilterSetupTemplate2(SigGroupHead *sgh);
...
static inline int Template2Match(const uint8_t parg2, const uint8_t mode,
        const uint8_t darg21, const uint8_t darg22)
{
    if (mode == DETECT_TEMPLATE2_EQ && parg2 == darg21)
        return 1;
    else if (mode == DETECT_TEMPLATE2_LT && parg2 < darg21)
        return 1;
    else if (mode == DETECT_TEMPLATE2_GT && parg2 > darg21)
        return 1;
    else if (mode == DETECT_TEMPLATE2_RA && (parg2 > darg21 && parg2 < darg22))
        return 1;
...
static int DetectTemplate2Setup (DetectEngineCtx *de_ctx, Signature *s, const char *template2str)
{
    ...
    template2d = DetectTemplate2Parse(template2str);
    if (template2d == NULL)
        goto error;
...
static int PrefilterSetupTemplate2(SigGroupHead *sgh)
{
    return PrefilterSetupPacketHeader(sgh, DETECT_TEMPLATE2,
